...
| **AI review** | Maintainer posts `/review` → Polly (`polly-review.yml`) reviews with full secrets; Copilot requested via `copilot-review.yml` and/or the maintainer's one-click **Reviewers → Copilot** button on the PR | Wired up | Keep. Polly **can't** auto-run on a fork `pull_request` (no secrets), so it's triggered by a maintainer `/review` comment, which runs trusted (default-branch checkout + diff via API, never executing PR code). Copilot can be requested by the maintainer in one click (add `@copilot` as a reviewer) or auto-requested by the workflow; either way it only produces a review once the org "allow unlicensed contributors" policy is on |
...
- **AI review trigger on forks** — the secret-bearing Polly review can't fire automatically on a fork `pull_request` (GitHub withholds repo secrets), so the working trigger is a maintainer (`OWNER`/`MEMBER`/`COLLABORATOR`) posting a `/review` comment; the `issue_comment` run executes in the trusted base context (default-branch checkout + diff fetched via API, never PR code). Copilot can be triggered either by its auto-request workflow on open or by the maintainer in one click (the **Reviewers → Copilot** button / add `@copilot` as a reviewer on the PR), but either way only yields a review once the org enables *"allow members without a Copilot license to use Copilot code review"*. If we want Polly to auto-run on fork PRs too, switch its trigger to `pull_request_target` (same trusted pattern the security gate uses) — at the cost of a full review spinning up on every fork push.
